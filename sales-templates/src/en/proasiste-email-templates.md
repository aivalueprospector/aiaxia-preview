
# ProAsiste Sales Email Templates

Copy, customize, and send.

---

## Initial Outreach

### Template 1: The Time-Saver Hook

**Subject**: Quick question about [Company Name]'s knowledge management

Hi [First Name],

Quick question: How many hours does your team spend each week searching for information across different systems?

Most operations teams I talk to estimate 5-10 hours per person per week. That's over 250 hours per year, just looking for things.

ProAsiste is an AI-powered knowledge system that captures everything your company knows and makes it instantly searchable. Ask a question, get an answer with sources in seconds.

Would you be open to a 15-minute call to see if this would be valuable for [Company Name]?

Best,
[Your Name]

---

### Template 2: The Competitor Angle

**Subject**: How [Competitor] is getting ahead

Hi [First Name],

I've been working with companies in [Industry] who are concerned about falling behind on AI adoption. Their competitors are using AI to move faster, not because of hype, but because finding information in seconds instead of hours is a real advantage.

ProAsiste is how companies like [Reference Customer] capture institutional knowledge and make it instantly accessible to their teams. It doesn't replace people. It multiplies what they can do.

I'd love to show you what this looks like in practice. Do you have 15 minutes this week?

Best,
[Your Name]

---

### Template 3: The Knowledge-Loss Problem

**Subject**: What happens when [Name] retires?

Hi [First Name],

Here's a question I ask every operations leader: What happens when your most experienced people leave?

Most companies lose decades of institutional knowledge overnight. The informal knowledge (the "ask Maria, she knows" kind) just disappears.

ProAsiste captures that knowledge continuously, turning conversations, documents, and processes into a permanent, searchable knowledge graph. When people leave, their knowledge stays.

Would it be valuable to chat about how this could work at [Company Name]?

Best,
[Your Name]

---

## Follow-Up After Demo

### Template 4: Post-Demo Summary

**Subject**: ProAsiste demo recap + next steps

Hi [First Name],

Thanks for taking the time to see ProAsiste today. Here's a quick recap of what we covered:

**Key Capabilities:**
- AI chat that knows YOUR business data
- Knowledge graph connecting documents, concepts, and processes
- Automated task execution for recurring workflows
- Document intelligence that extracts and connects information
- Process mining to find hidden bottlenecks

**What You Seemed Most Interested In:**
[Customize based on demo, e.g., "The ability to automate weekly reports" or "Capturing knowledge before key employees retire"]

**Potential ROI We Discussed:**
[Customize, e.g., "If each team member saves 5 hours/week, that's 250 hours/year at $50/hour = $12,500 per person"]

**Next Steps:**
1. [Action item, e.g., "I'll send over the security documentation you requested"]
2. [Action item, e.g., "Let's schedule a follow-up with your IT team"]
3. [Action item, e.g., "Trial period starting next Monday"]

If anything's unclear, just reply and ask.

Best,
[Your Name]

---

### Template 5: No Response Follow-Up

**Subject**: Re: ProAsiste demo recap + next steps

Hi [First Name],

Just following up on our demo last week. I know things get busy.

Is ProAsiste still something you're considering? If timing isn't right, totally understand. Just let me know either way so I can update my notes.

If you have questions or concerns, happy to address those too.

Best,
[Your Name]

---

## Objection Response Emails

### Template 6: Budget Concerns

**Subject**: Re: ProAsiste pricing question

Hi [First Name],

Thanks for your honesty about budget concerns. Let me reframe the cost:

**The Investment:**
[Tier] plan: $X/month

**The Return:**
- 5 hours saved per person per week = 260 hours/year
- At $50/hour fully loaded cost = $13,000 saved per person
- For a team of 10 = $130,000 in time savings alone

That's not counting the value of:
- Knowledge retention when employees leave
- Faster onboarding of new team members
- Reduced errors from better information access

The ROI typically pays for ProAsiste in the first month.

Would it help to start with a smaller pilot team to prove the value before expanding?

Best,
[Your Name]

---

### Template 7: Security Concerns

**Subject**: Re: Data security questions

Hi [First Name],

Security matters. Here's exactly how ProAsiste protects your data:

**Data Isolation:**
- Your ProAsiste runs in a dedicated environment
- Your data is never commingled with other customers
- Your data NEVER trains public AI models

**Encryption:**
- Data encrypted in transit (TLS 1.3)
- Data encrypted at rest (AES-256)

**Access Control:**
- Token-based authentication
- Role-based permissions for ORG twins
- Full audit trail of all access

**Data Ownership:**
- You own your data, period
- Export anytime in standard formats
- Delete anytime with full removal

I can also connect you with our security team for a deeper technical discussion, or provide our SOC 2 documentation.

Would that be helpful?

Best,
[Your Name]

---

### Template 8: "We'll Build It Ourselves"

**Subject**: Re: Build vs. buy decision

Hi [First Name],

Building in-house is definitely an option. Here's the honest comparison:

**Building Custom:**
- Development time: 3-12 months
- Development cost: $200K-$1M+ (team of 3-5 engineers for 6+ months)
- Ongoing maintenance: 1-2 FTEs
- Knowledge graph: Build from scratch
- UI/UX: Build from scratch
- Keeping up with AI advances: Your responsibility

**ProAsiste:**
- Deployment: 1 day
- Cost: $X/month subscription
- Maintenance: Included
- Knowledge graph: Built-in
- UI/UX: Included
- AI updates: Automatic

**The Real Question:**
Do you want your engineering team building AI infrastructure, or building your actual product?

Happy to chat through the tradeoffs in more detail if helpful.

Best,
[Your Name]

---

## Closing / Urgency Emails

### Template 9: End of Quarter Push

**Subject**: Q[X] pricing ending [Date]

Hi [First Name],

Quick heads up: Our Q[X] pricing ends [Date]. After that, [Tier] pricing increases from $X to $Y.

If you're still considering ProAsiste, locking in now saves $[Amount]/year.

No pressure. Just wanted to make sure you had the information.

Let me know if you'd like to move forward, or if you have any final questions.

Best,
[Your Name]

---

### Template 10: Pilot Proposal

**Subject**: Low-risk way to test ProAsiste

Hi [First Name],

I know making a decision on new technology can feel risky. Here's a low-risk way to test ProAsiste:

**30-Day Pilot:**
- Start with 3-5 users in one department
- Full access to [Tier] features
- Our team helps with setup and training
- After 30 days, review results together
- No commitment to continue if it's not working

This way you can see real results with real data before any larger commitment.

Interested in giving it a try?

Best,
[Your Name]

---

## Re-Engagement Emails

### Template 11: Market Update Re-Engagement

**Subject**: [Industry] companies adopting AI faster than expected

Hi [First Name],

We spoke a few months ago about ProAsiste. Since then, I've been working with several [Industry] companies and noticed a trend worth sharing:

AI adoption is accelerating faster than expected. Companies that started 6 months ago now have:
- Thousands of documents in their knowledge graph
- Dozens of automated workflows
- Months of conversation history the AI has learned from

The companies that wait are falling further behind, because knowledge compounds over time and an early start is hard to close.

If the timing is better now, I'd love to reconnect. If not, no worries. Just wanted to share what I'm seeing in the market.

Best,
[Your Name]

---

### Template 12: New Feature Announcement

**Subject**: New ProAsiste feature you asked about

Hi [First Name],

Remember when we spoke and you asked about [Feature they wanted]?

Good news: we just launched it.

[Brief description of feature]

Would you be open to a quick 15-minute call to see how this addresses what you were looking for?

Best,
[Your Name]

---

## Email Best Practices

### Subject Line Rules
- Keep under 50 characters
- Make it specific to them
- Avoid spam triggers (FREE, LIMITED TIME, ACT NOW)
- Questions often outperform statements

### Body Rules
- Keep first email under 150 words
- One clear call to action
- Make it easy to say yes or no
- Personalize with company name, industry, or recent news

### Timing
- Tuesday-Thursday typically best
- 8-10 AM or 2-4 PM local time
- Avoid Mondays (inbox overload) and Fridays (weekend mode)

### Follow-Up Cadence
- Day 1: Initial email
- Day 3: Follow-up if no response
- Day 7: Different angle/value prop
- Day 14: Final attempt
- Then: Move to nurture sequence

